...
const
  ...
  GSMConfigAvail : array[0..7] of string =
              (//'ZE0\Q3',                // Turn off echo
               'Z',                // Init
               'E0+CMEE=2',         // Turn off echo
  ...
  GSMConfigResponse : array[0..7] of string =
  ...
  GSMDiagnostic : array[0..0] of string =
               ('I');          // Send Message from Storage
  GSMRegistration : array[0..0] of string =
               ('+COPS?+CREG?');    // Send Message from Storage
...
begin
  ...
  if FNotifyOnNewMessage and not Assigned(NotifyPacket) then begin{!!.02}{!!.05}
    ...
    NotifyPacket.EndString := #13#10;                                       {!!.05}
  ...
  NewTimer(ET, 1080 div 12); // 60/12 second timer
  repeat
    Res := DoSafeYield;
...
procedure TApdCustomGSMPhone.Diagnose;
var
  Res : Integer;
  ET : EventTimer;
begin
  ...
  try
    ...
    ResponsePacket.EndString := #13#10'OK'#13#10;
    ResponsePacket.ComPort := FComPort;
    ResponsePacket.Enabled := True;
    //DelayTicks(4, True);                                                 {!!.04}
    FComPort.Output := 'ATI'#13#10;
    NewTimer(ET, 18*3); // 3 second timer
    repeat
      Res := DoSafeYield;
    until (FGSMState = gsNone) or (Res = wm_Quit) or TimerExpired(ET);
  finally
...
function ResParam(Res:String;Prefix:String;Index:Integer):String;
var S:TStrings;
begin
...
function TApdCustomGSMPhone.Registration;
var
  Res : Integer;
  ET : EventTimer;
begin
  ...
  try
    ...
    ResponsePacket.Enabled := True;
    //DelayTicks(4, True);                                                 {!!.04}
    FComPort.Output := 'AT+COPS?+CREG?'#13#10;
    NewTimer(ET, 18*3); // 3 second timer
    repeat
      Res := DoSafeYield;
    until (FGSMState = gsNone) or (Res = wm_Quit) or TimerExpired(ET);
  ...
  if ResParam(ResponseStr,'+CREG',1)='1' then
    Result:=ResParam(ResponseStr,'+COPS',2)
  else
    Result:='';
end;
...
begin
  ...
  case FGSMState of
    ...
    gsDiagnose : begin
                     inc(CmdIndex);
                     if CmdIndex > High(GSMDiagnostic) then begin
                       FGSMState := gsNone;
                       ResponseStr := '';                                {!!.02}
                     end else
                       FComPort.Output := 'AT' +
                                     GSMDiagnostic[CmdIndex] + #13;
                   end;
    gsRegistration : begin
                     inc(CmdIndex);
                     if CmdIndex > High(GSMRegistration) then begin
                       FGSMState := gsNone;
                       //ResponseStr := '';                                {!!.02}
                     end else
                       FComPort.Output := 'AT' +
                                     GSMRegistration[CmdIndex] + #13;
                   end;
  end; // End Case
end;
...
begin
  ...
  if Assigned(FComPort) then begin
    ...
    if not Assigned(ErrorPacket) then begin                              {!!.05}
      ...
      ErrorPacket.EndString := #13#10;                                      {!!.05}
...
function TApdCustomGSMPhone.DoSafeYield: LongInt;
begin
  Result:=SafeYield;
  if Result=0 then Sleep(30);
end;
