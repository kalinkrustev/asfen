...
Type
  ...
  Private
    ...
    Procedure SetCheckList(Value: TStrings);
  Protected
    Procedure DoChecked(Node: PVirtualNode); Override;
    Procedure DoReadNodeFromDB(Node: PVirtualNode); Override;
  Public
    Constructor Create(Owner: TComponent); Override;
    Destructor Destroy; Override;
    Property CheckList: TStrings Read GetCheckList Write SetCheckList;
  End;
    ...
    If Parent <> RootNode Then Begin
      ...
      If (dboPathStructure In FDBOptions) Or (dboWriteSecondary In FDBOptions) Then Begin
        ...
        While Parent <> RootNode Do Begin
          Data := GetNodeData(Parent);
          Path := Format('%1.0f.%s', [Data.ID, Path]);
          Parent := Parent.Parent;
        End;
      End;
    End;
...
Function TCustomCheckVirtualDBTreeEx.GetCheckList: TStrings;
Begin
  Result := TStringList.Create;
  Result.Assign(FList);
End;

Procedure TCustomCheckVirtualDBTreeEx.SetCheckList(Value: TStrings);
Begin
  FList.Assign(Value);
  UpdateTree;
End;
