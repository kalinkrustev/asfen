Ext.onReady(function(){
    ...
    var grid = new Ext.grid.GridPanel({
        stateful:true,
        stateId:'monitoringorganisations',
        border: false,
        region: 'center',
        store: store,
        columns: [
    ...
    grid.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
        detailstemplate.overwrite(Ext.getCmp('ringdetails').body,r.data);
	});
    ...
    reportstatus.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
        journal2.setRawValue(r.data.journal);
        detailstemplate.overwrite(Ext.getCmp('reportdetails').body,r.data);
	});
    ...
    eventgrid.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
        ringlist.removeAll();
        ringlist.load({
            params:{mode:eventlist.mode,
                    eventid:sm.hasSelection()?sm.getSelected().id:0
            }
        });
	});
    ...
    reportgrid.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {
        ...
        report.load({
            params:{mode:reportlist.mode,
                    fileid:sm.hasSelection()?sm.getSelected().id:0
            }
        });
	});
                    ...
                    handler  : function(){
                        Ext.ComponentMgr.get("modepanel").getForm().submit({
                           ...
                           success:function(form,action){
                             modewindow.hide();
                             if (tabs.getActiveTab() && tabs.getActiveTab().fireEvent)
                                tabs.getActiveTab().fireEvent('activate');
                           },
        ...
        ,
            new Ext.Panel({
                ...
                items:[{
                    ...
                    margins: '5 5 0 0',
                    layout: "border",
                    items:[grid,{title:'детайли',region:'south',height:90,id:'ringdetails',border:false}]
                }]
                })
        ,
            new Ext.Panel({
                ...
                ,{ 
                    ...
                    margins: '5 5 0 0',
                    layout: "border",
                    items: [reportstatus,{title:'детайли',region:'south',height:90,id:'reportdetails',border:false}],
