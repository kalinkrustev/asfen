...
function tObjList.Empty:boolean;
  begin
  Result:=head^.r=z;
  end;

function tObjList.New_Node(const k:itemType; const i:infoType; const bi:boolean; const ll,rr:pnode):pnode;
  var
    a:longint;
    p:pnodearr;
  begin
...
procedure tObjList.PoolSize(size:integer);
// PoolSize;
  begin
...
procedure tObjList.Del_Node(node:pnode);
// del_node
  begin
  if myPoolSize>0 then
    pool.Put(node)
...
constructor tObjList.Create(size:integer);
// Create
  begin
  inherited Create;
  cnt:=0;
...
procedure tObjList.Change(const v:itemType;const info:infoType);
// Change
  var
    x:pnode;
  begin
...
function tObjList.Search(const v:itemType):infoType;
// Search
  var
    x:pnode;
  begin
...
function tObjList.Search_Min(var i:infoType):itemType;
// Search_Min
  var
    x:pnode;
  begin
...
function tObjList.Search_Max(var i:infoType):itemType;
// Search_Max
  var
    x:pnode;
  begin
...
function tObjList.Search_L(const v:itemType; var i:infoType):itemType;
// Search_L
  var
    x,y:pnode;
  begin
...
function tObjList.Search_G(const v:itemType; var i:infoType):itemType;
// Search_G
  var
    x,y:pnode;
  begin
...
function tObjList.Search_LE(const v:itemType; var i:infoType):itemType;
// Search_LE
  var
    x,y:pnode;
  begin
...
function tObjList.Search_GE(const v:itemType; var i:infoType):itemType;
// Search_GE
  var
    x,y:pnode;
  begin
...
procedure tObjList.Insert(const v:itemType;const info:infoType);
// Insert
  var
    nx,x,p,g,gg,c:pnode;
  procedure split;
...
procedure tObjList.Remove(const v:itemType);
// Remove
  var
    cb:boolean;
...
function tObjList.Count: cardinal;
  begin
  Result:=cnt;
  end;
